...
//FROM MDN: An arrow function expression is a compact alternative to a traditional function expression, but is limited and can't be used in all situations.
// Does not have its own bindings to this.
// So because an arrow function has no binding to (this) when we attempt to use an arrow function with a Constructor and then attempt to invoke the information like we did with our consol.log we get a returned value of undefined as the value has nothing assigned as you can't bind to (this) when using an arrow function
